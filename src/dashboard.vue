...
<template>
  <div class="main">
    ...
    <div
      ...
      :style="{ width: secondMenu.length > 1 ? '200px' : '90px' }"
    >
          ...
          <template v-for="(item, index) in secondMenu">
            <div :key="index" v-if="item.menu">
              <div
                class="menu-span"
                :class="
                  $route.name === item.name || $route.meta.id === item.name
                    ? 'active'
                    : ''
                "
                @click="navTo(item)"
    ...
    <div
      ...
      :style="{ left: secondMenu.length > 1 ? '200px' : '90px' }"
    >
      <!--主内容标签页start-->
        ...
        <div class="main-header">
          <div class="header-middle-con">
            <template v-if="secondMenu.length <= 1">
              <div class="show-name">{{ companyName }}</div>
            </template>
            <template v-else>
              ...
              >
                ...
                >
                  ...
                  <DropdownMenu slot="list">
                    <DropdownItem
                      name="ownSpace"
                      @click.native="opendChangePasswordModal"
                      >修改密码</DropdownItem
                    >
...
export default {
    ...
    return {
      ...
      formDataRules: {
        newPassword: [
          {
            required: true,
            message: '新密码不能为空,请使用数字、英文或特殊字符两种及以上！',
          },
          {
            pattern: reg,
            message: '密码至少包含数字、英文或特殊字符其中的两种!',
          },
        ],
        confirmPassword: [
          {
            required: true,
            message: '确认密码不能为空,请使用数字、英文或特殊字符两种及以上！',
          },
          {
            pattern: reg,
            message: '密码至少包含数字、英文或特殊字符其中的两种!',
  ...
  methods: {
    ...
    initMenuSelected(route) {
      ...
      const firstMenuItem =
        secondMenuItem && secondMenuItem.getParent && secondMenuItem.getParent()

      let menus = []
      if (firstMenuItem) {
        this.shrinkTitle = firstMenuItem.title
        firstMenuItem.children &&
          firstMenuItem.children.forEach((item) => {
