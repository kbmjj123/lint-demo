	...
	actions: {
		...
		loginByPassword ({ commit, dispatch }, userInfo) {
			return new Promise((resolve, reject) => {
				api.user.loginByPassword(userInfo)
					.then(res => {
						if (res.result === '000') {
							...
							window.localStorage.wmUserInfo = JSON.stringify({ userId: res.data.memberId, userTag: '中后台', projectVersion: '1.0.0' });
		...
		loginByRandomCode({ commit, dispatch }, randomInfo) {
			return new Promise((resolve, reject) => {
				api.user.randomCodeLogin(randomInfo).then(res => {
					if('000' === res.result){
						Cookies.set('account', res.data.account);
						Cookies.set('token', res.data.token);
						Cookies.set('haveShop', res.data.haveShop);
