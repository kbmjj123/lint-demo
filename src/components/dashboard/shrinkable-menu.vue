...
<template>
	<div :style="{background: bgColor}" class="ivu-shrinkable-menu">
		<slot name="top"></slot>
		<Menu :style="{maxHeight: menuMaxHeight, overflowX: 'hidden', overflowY: 'scroll'}" ref="sideMenu" :active-name="routerName" :theme="theme" width="auto" @on-select="changeMenu">
			<template v-for="item in menuList">
				<MenuItem :name="item.name" v-if="item.menu" :key="'menuitem' + item.name">
					<span :class="['icon',item.icon, routerName === item.name ? item.icons : '']" :style="{fontSize:'16px'}"></span>
					<span class="layout-text" style="line-height:1.5" :key="'title' + item.name">{{item.title}}</span>
				</MenuItem>
			</template>
		</Menu>
	</div>
...
<script>
	...
	export default {
		...
		methods: {
			changeMenu(value) {
				if ((value !== this.routerName) || (value === this.routerName && value === 'marketing')) {
					this.$emit('on-change', (this.menuItem.children && this.menuItem.children.length > 1) ? this.menuItem.children : [], this.menuItem);
					this.$router.push({
						name: value
					});
				}
